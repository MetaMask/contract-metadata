...
test("logos path names should match exactly", function (t) {
  ...
  Object.keys(contractMap).forEach((address) => {
    ...
    const fileName = contract.logo;
    t.ok(
      dirContent.includes(fileName),
      `filename matches exactly: "${fileName}"`
    );
  });
...
test("generated modules file should match contract json", function (t) {
  ...
  t.ok(
    Object.keys(contractMap).length >= Object.keys(generatedModules).length,
    `logo modules count doesn't exceed contract map`
  );
  t.end();
});
